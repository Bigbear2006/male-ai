...
async def schedule_handler(query: CallbackQuery, state: FSMContext):
    ...
    try:
        schedule = await Schedule.objects.prefetch_related('time_blocks').aget(
...
async def set_schedule_type(query: CallbackQuery):
    ...
    schedule, _ = await Schedule.objects.aupdate_or_create(
...
async def edit_time_block_field(query: CallbackQuery, state: FSMContext):
    ...
    texts = {
        ...
        'start_time': 'Укажи время начала блока',
        'end_time': 'Укажи время завершения блока',
    }
...
async def set_time_block_name(msg: Message, state: FSMContext):
    ...
    await msg.answer('Укажи время начала блока')
